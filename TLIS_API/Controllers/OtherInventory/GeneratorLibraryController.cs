...
namespace TLIS_API.Controllers.OtherInventory
{
    ...
    public class GeneratorLibraryController : ControllerBase
    {
        ...
        public async Task<IActionResult> DisableGeneratorLibrary(int Id)
        {
            ...
            string userInfo = jsonToken.Claims.First(c => c.Type == "sub").Value;
            var userId = Convert.ToInt32(userInfo);
            var response = await _unitOfWorkService.OtherInventoryLibraryService.Disable(Id, Helpers.Constants.OtherInventoryType.TLIgeneratorLibrary.ToString(), ConnectionString,userId);
            return Ok(response);
        }
        ...
        public async Task<IActionResult> DeleteGeneratorLibrary(int Id)
        {
            ...
            string userInfo = jsonToken.Claims.First(c => c.Type == "sub").Value;
            var userId = Convert.ToInt32(userInfo);
            var response = await _unitOfWorkService.OtherInventoryLibraryService.Delete(Id, Helpers.Constants.OtherInventoryType.TLIgeneratorLibrary.ToString(), ConnectionString,userId);
            return Ok(response);
        }
