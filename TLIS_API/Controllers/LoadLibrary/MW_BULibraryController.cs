   ...
   // [ServiceFilter(typeof(MiddlewareLibraryAndUserManagment))]
    ...
    public class MW_BULibraryController : Controller
    {
        ...
        public async Task<IActionResult> DisableMW_BULibrary(int Id)
        {
            ...
            string userInfo = jsonToken.Claims.First(c => c.Type == "sub").Value;
            var userId = Convert.ToInt32(userInfo);
            var response = await _unitOfWorkService.MWLibraryService.Disable(Id, Helpers.Constants.LoadSubType.TLImwBULibrary.ToString(), ConnectionString, userId);
            return Ok(response);
        }
        ...
        public async Task<IActionResult> DeleteMW_BULibrary(int Id)
        {
            ...
            string userInfo = jsonToken.Claims.First(c => c.Type == "sub").Value;
            var userId = Convert.ToInt32(userInfo);
            var response = await _unitOfWorkService.MWLibraryService.Delete(Id, Helpers.Constants.LoadSubType.TLImwBULibrary.ToString(), ConnectionString, userId);
            return Ok(response);
        }
