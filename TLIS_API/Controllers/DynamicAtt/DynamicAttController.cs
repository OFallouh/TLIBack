...
namespace TLIS_API.Controllers.DynamicAtt
{
    ...
    public class DynamicAttController : Controller
    {
        ...
        public IActionResult GeStaticAttsAndDynamicAttsByTableName( String TabelName,bool IsLibrary,int? CategoryId)
        {
            ...
            var userId = Convert.ToInt32(userInfo);
            var ConnectionString = _configuration["ConnectionStrings:ActiveConnection"];
            var response = _unitOfWorkService.DynamicAttService.GeStaticAttsAndDynamicAttsByTableName(TabelName, IsLibrary, CategoryId,userId,false);
            return Ok(response);
        }
        ...
        public IActionResult getLayersAndAttributesStaticAndDynamic(String TabelName, int? CategoryId)
        {
            ...
            var userId = Convert.ToInt32(userInfo);
            var ConnectionString = _configuration["ConnectionStrings:ActiveConnection"];
            var response = _unitOfWorkService.DynamicAttService.getLayersAndAttributesStaticAndDynamic(userId,TabelName, CategoryId,false);
            return Ok(response);
        }
        [HttpGet("GetById")]
