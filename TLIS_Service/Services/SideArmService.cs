        ...
        public Response<GetForAddLoadObject> GetById(int Id,int UserId,bool Externalys)
        {
            try
            {
                ...
                if (sideArm != null)
                {
                    ...
                    if (NameAttribute != null)
                    {
                        NameAttribute.Value = _dbContext.MV_SIDEARM_VIEW.FirstOrDefault(x => x.Id == Id)?.Name;
                    }
                   ...
                   .ToList();
                    ...
                    if (sideArm.allCivilInst != null)
                    {
                        ...
                        if (sideArm.allCivilInst.civilWithoutLegId != null)
                        {
                            ConfigureView1("civilWithoutLeg", sectionsLegTypeViewModels[0], "civilWithoutLegId", sideArm.allCivilInst.civilWithoutLeg, _unitOfWork.CivilWithoutLegRepository.GetWhere(x => x.Id == sideArm.allCivilInst.civilWithoutLegId));

                        }
                        else if (sideArm.allCivilInst.civilNonSteelId != null)
                        {
                            ConfigureView2("civilNonSteel", sectionsLegTypeViewModels[1], "civilNonSteelId", sideArm.allCivilInst.civilNonSteel, _unitOfWork.CivilNonSteelRepository.GetWhere(x => x.Id == sideArm.allCivilInst.civilNonSteelId));
                        }
                        else if (sideArm.allCivilInst.civilWithLegsId != null)
                        {
                            ConfigureView3("civilWithLeg", sectionsLegTypeViewModels[2], "civilWithLegId", sideArm.allCivilInst.civilWithLegs, _unitOfWork.CivilWithLegsRepository.GetWhere(x => x.Id == sideArm.allCivilInst.civilWithLegsId));
                        }
                        ...
                        if ((sideArm.legId ?? 0) != 0 || (sideArm.Leg2Id ?? 0) != 0)
                        {
                            List<SectionsLegTypeViewModel> sectionsLegTypeViewModelLeg = new List<SectionsLegTypeViewModel>();
                            List<int> ints = new List<int>();

                            void AddLegDetails(int? legId)
                            {
                                if (legId.HasValue && legId.Value != 0)
                                {
                                    var leg = _unitOfWork.LegRepository.GetWhereFirst(x => x.Id == legId.Value);
                                    if (leg != null)
                                    {
                                        sectionsLegTypeViewModelLeg.Add(new SectionsLegTypeViewModel
                                        {
                                            Id = leg.Id,
                                            Name = leg.CiviLegName
                                        });
                                        ints.Add(leg.Id);
                                    }
                                }
        ...
        public async Task<Response<EditSidearmInstallationObject>> UpdateSideArm(EditSidearmInstallationObject SideArmViewModel, int? TaskId, int UserId, string ConnectionString, bool ExternalSys)
        {
            using (TransactionScope transactionScope = new TransactionScope())
            {
                try
                {
                    ...
                    , x => x.allCivilInst.civilWithoutLeg,x=>x.sideArm);
                    if (CivilLoads != null)
                    {
                        ...
                        var RelatedtoSideArm = _dbContext.TLIcivilLoads.Where(
                          x =>
                              (x.sideArmId == SideArmViewModel.installationAttributes.Id
                               || x.sideArm2Id == SideArmViewModel.installationAttributes.Id)
                              &&
                        ...
                        if (SideArmViewModel.installationConfig.civilSteelType == 0)
                        {
                            if (SideArmViewModel.installationConfig.civilWithLegId != 0 || SideArmViewModel.installationConfig.civilWithLegId != null)
                            {
                                        ...
                                        if (SideArmViewModel.installationConfig.installationPlaceId == 1)
                                        {
                                            if (SideArmViewModel.installationConfig.legId.Count == 1)
                                            {
                                                ...
                                                if (civilwithlegname != null)
                                                {
                                                   ...
                                                   , null, SideArmViewModel.installationConfig.civilWithLegId, null, null, null, null, SideArm.Azimuth
                                        ...
                                        if (SideArmViewModel.installationConfig.installationPlaceId == 1 || SideArmViewModel.installationConfig.installationPlaceId == 2)
                                        {
                                            if (SideArmViewModel.installationConfig.legId.Count == 2)
                                            {
                                                ...
                                                if (civilwithlegname != null)
                                                {
                                                    var Data = _unitOfWork.CivilWithLegsRepository.EditFilterAzimuthAndHeight(null, null, null,
                                                    null, null, null, null, null, null, null, SideArm.Id, "TLIsideArm", CivilLoads.SiteCode, SideArmViewModel.installationConfig.legId[0]
                                                   , SideArmViewModel.installationConfig.legId[1], SideArmViewModel.installationConfig.civilWithLegId, null, null, null, null, SideArm.Azimuth
                                                   , SideArm.HeightBase, 5).Data;
                                ...
                                {
                                    return new Response<EditSidearmInstallationObject>(false, null, null,
                                  "this civil is not found", (int)ApiReturnCode.fail);
                                }
                            }
                            else
                            {
                        ...
                        else if (SideArmViewModel.installationConfig.civilSteelType == 1)
                        {
                            ...
                            if (SideArmViewModel.installationConfig.civilWithoutLegId != 0 || SideArmViewModel.installationConfig.civilWithoutLegId != null)
                            {
                                ...
                                SideArmViewModel.installationConfig.civilWithoutLegId);
                                if (ALLCivil != null)
                                {
                                    AllCivilInst = ALLCivil.Id;
                                    if (SideArmViewModel.installationConfig.sideArmTypeId == 1)
                        ...
                        else if (SideArmViewModel.installationConfig.civilSteelType == 2)
                        {
                            ...
                            {
                                var ALLCivil = _unitOfWork.AllCivilInstRepository.GetIncludeWhereFirst(x => x.civilNonSteelId ==
                                 SideArmViewModel.installationConfig.civilNonSteelId);
                                if (ALLCivil != null)
                                {
                                    AllCivilInst = ALLCivil.Id;
                                    if (SideArmViewModel.installationConfig.sideArmTypeId == 1)
                        ...
                        //if (!string.IsNullOrEmpty(CheckDependencyValidation))
                        //    return new Response<AllItemAttributes>(true, null, null, CheckDependencyValidation, (int)ApiReturnCode.fail);
                        TLIsideArm SideArm1 = _unitOfWork.SideArmRepository.GetWhereFirst(x => x.Id == SideArmViewModel.installationAttributes.Id);
                        if (SideArm1 == null)
                        {
                            return new Response<EditSidearmInstallationObject>(false, null, null, "This sidearm is not found", (int)ApiReturnCode.fail);
                        }

                        SideArm1.sideArmInstallationPlaceId = SideArmViewModel.installationConfig.installationPlaceId;
                        SideArm1.sideArmTypeId = SideArmViewModel.installationConfig.sideArmTypeId;
                        SideArm1.sideArmLibraryId = SideArmViewModel.civilType.sideArmLibraryId;

                        _mapper.Map(SideArmViewModel.installationAttributes, SideArm1); //ربط الخصائص من نموذج العرض بالكيان الحالي.

                        var HistoryId = _unitOfWork.SideArmRepository.UpdateWithHInstallation(UserId, null, SideArmInst, SideArm1, CivilLoads.SiteCode, ExternalSys);
                        _unitOfWork.SaveChanges();
            ...
            using (TransactionScope transaction = new TransactionScope())
            {
                try
                {
                    ...
                    if (SideArmLibrary != null)
                    {
                                        ...
                                        if (addSideArms.installationConfig.legId.Count == 1)
                                        {
                                            ...
                                            if (civilwithlegname != null)
                                            {
                                                var Data = _unitOfWork.CivilWithLegsRepository.FilterAzimuthAndHeight(SiteCode, addSideArms.installationConfig.legId[0]
                                                           , null, addSideArms.installationConfig.civilWithLegId, null, null, null, null, SideArm.Azimuth
                                                           , SideArm.HeightBase, 4).Data;

                                                if (Data == false)
                                                {
                                                    return new Response<SideArmViewDto>(false, null, null, "can not installed the SideArm on same azimuth and height because found other SideArm in same angle", (int)ApiReturnCode.fail);
                                                }

                                                var LegLetter = _unitOfWork.LegRepository.GetWhereFirst(x => x.Id == addSideArms.installationConfig.legId[0])?.LegLetter;
                                                if (civilwithlegname != null && LegLetter != null)
                                                {
                                ...
                                else if (addSideArms.installationConfig.sideArmTypeId == 2)
                                {
                                    if (addSideArms.installationConfig.installationPlaceId == 1 || addSideArms.installationConfig.installationPlaceId == 2)
                                    {
                                        if (addSideArms.installationConfig.legId.Count == 2)
                                        {
                                            ...
                                            {
                                                var Data = _unitOfWork.CivilWithLegsRepository.FilterAzimuthAndHeight(SiteCode, addSideArms.installationConfig.legId[0]
                                                             , addSideArms.installationConfig.legId[1], addSideArms.installationConfig.civilWithLegId, null, null, null, null, SideArm.Azimuth
                                                             , SideArm.HeightBase, 5).Data;
                                                ...
                                                var LegLetter = _unitOfWork.LegRepository.GetWhereFirst(x => x.Id == addSideArms.installationConfig.legId[0])?.LegLetter;
                                                var LegLetter2 = _unitOfWork.LegRepository.GetWhereFirst(x => x.Id == addSideArms.installationConfig.legId[1])?.LegLetter;
                                                if (LegLetter != null && LegLetter2 != null && civilwithlegname != null)
                                                {
                        ...
                        {
                            if (addSideArms.installationConfig.civilNonSteelId != 0 || addSideArms.installationConfig.civilNonSteelId != null)
                            {
                                if (addSideArms.installationConfig.sideArmTypeId == 1)
                                {
