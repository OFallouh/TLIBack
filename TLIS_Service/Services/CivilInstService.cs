...
namespace TLIS_Service.Services
{
    public class CivilInstService : ICivilInstService
    {
        ...
        #endregion
        public Response<ObjectInstAtts> AddCivilInstallation(AddCivilWithLegsViewModel AddCivilWithLegsViewModel, string TableName, string SiteCode, string connectionString,int? TaskId,int UserId)
        {
            ...
            using (TransactionScope transaction = new TransactionScope())
            {
                try
                {
                    ...
                    if (Helpers.Constants.CivilType.TLIcivilWithLegs.ToString() == TableName)
                    {
                        ...
                        {
                            var CheckSpace = _unitOfWork.SiteRepository.CheckSpace(SiteCode, TableName, AddCivilWithLegsViewModel.civilType.civilWithLegsLibId, civilWithLegs.SpaceInstallation, null).Message;
        ...
        public Response<GetForAddCivilWithLegObject> GetById(int CivilInsId, string TableName)
        {
            try
            {
                TLItablesNames TableNameEntity = _unitOfWork.TablesNamesRepository
                ...
                if (Helpers.Constants.CivilType.TLIcivilWithLegs.ToString() == TableName)
                {
                    ...
                    foreach (BaseInstAttViews LibraryAttribute in LibraryAttributes)
                    {
                        if (LibraryAttribute.DataType.ToLower() == "list")
                        {
                            LibraryAttribute.Value = CivilWithLegLibrary.GetType().GetProperties()
                    ...
                    var supportedAttributes = CivilsupportAttributes
                      .Where(attribute => attribute.DataType.ToLower() == "list" &&
                          attribute.Key.ToLower() == "referencecivilid" &&
                          civilSupportDistance != null);
                    if (supportedAttributes.Any())
                    {
                        ...
                        if (support != null)
                        {
                            ...
                            if (SupportReferenceAllCivilInst != null)
                            {
                                ...
                                var allOptions = allCivil.SelectMany(item =>
                                {
                                    var civilWithLegsOptions = _mapper.Map<List<SupportTypeImplementedViewModel>>(item.allCivilInst.civilWithLegs);
                                    var civilWithoutLegOptions = _mapper.Map<List<SupportTypeImplementedViewModel>>(item.allCivilInst.civilWithoutLeg);
                                    var civilNonSteelOptions = _mapper.Map<List<SupportTypeImplementedViewModel>>(item.allCivilInst.civilNonSteel);

                                    return civilWithLegsOptions.Concat(civilWithoutLegOptions).Concat(civilNonSteelOptions);
                                }).ToList();

                                foreach (var attribute in supportedAttributes)
                                {
                                    attribute.Value = SupportReferenceAllCivilInst.civilWithLegsId != null ? SupportReferenceAllCivilInst.civilWithLegs.Name :
        ...
        public Response<SideArmAndLoadsOnCivil> CheckFilterSideArm_LoadsOnCivils(int CivilId, string CivilType)
        {
            ...
            else if (CivilType == "TLIcivilWithoutLeg")
            {
                ...
                if (AllCivilInst != null)
                {
                    ...
                    AllCivilInst.civilWithoutLeg.CurrentLoads = null;
                    foreach (var item in AllLoadOnCivil)
                    {
                        if (item.allLoadInst != null)
                        {
            ...
            else if (CivilType == "TLIcivilWithoutLeg")
            {
                if (AllCivilInst.civilWithoutLeg != null)
                {
                    if (AllCivilInst.civilWithoutLeg.Support_Limited_Load != 0)
                    {
                        if (AllCivilInst.civilWithoutLeg.CurrentLoads != null)
                        {
                            Availablespace = AllCivilInst.civilWithoutLeg.Support_Limited_Load - AllCivilInst.civilWithoutLeg.CurrentLoads.Value;
                        }
                    ...
                    else
                    {
                        if (AllCivilInst.civilWithoutLeg.CurrentLoads != null)
                        {
                            Availablespace = AllCivilInst.civilWithoutLeg.CivilWithoutlegsLib.Manufactured_Max_Load - AllCivilInst.civilWithoutLeg.CurrentLoads.Value;
                        }
        ...
        public Response<GetForAddCivilWithLegObject> GetForAddCivilWithLegInstallation(string TableName, int CivilLibraryId,string SiteCode)
        {
            try
            {
                ...
                {
                    { "civilsteelsupportcategory_name", () =>
        ...
        public Response<GetForAddCivilWithOutLegInstallationcs> GetForAddCivilWithOutLegInstallation_Capsule(string TableName, int CivilLibraryId, string SiteCode)
        {
            try
            {
                ...
                List<BaseInstAttViews> LibraryAttributes = _unitOfWork.AttributeActivatedRepository
                    .GetAttributeActivatedGetForAdd(Helpers.Constants.TablesNames.TLIcivilWithoutLegLibrary.ToString(), null, null).ToList();
                if (CivilWithoutLegLibrary.structureTypeId != null || CivilWithoutLegLibrary.structureTypeId != 0)
                {
                    structureType = _unitOfWork.StructureTypeRepository.GetWhereFirst(x => x.Id == CivilWithoutLegLibrary.structureTypeId);
                }
