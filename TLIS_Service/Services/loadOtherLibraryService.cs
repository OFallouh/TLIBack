...
{
    public class LoadOtherLibraryService : ILoadOtherLibraryService
    {
        private readonly IUnitOfWork _unitOfWork;
        IServiceCollection _services;
        private IMapper _mapper;

        public static List<TLIattributeActivated> _LoadOtherLibraryAttributeActivated;
        public static List<TLIdynamicAtt> _LoadOtherLibraryDynamicAttributes;
        public static List<TLIloadOtherLibrary> _LoadOtherLibraryEntities;
        public static List<TLIattributeViewManagment> _LoadOtherLibraryAllAttributesViewManagement;
                ...
                using (var tran = con.BeginTransaction())
                {
                            ...
                            new System.TimeSpan(0, 15, 0)))
                    {
                        try
                        {
                            ...
                            TLItablesNames TableNameEntity = _unitOfWork.TablesNamesRepository.GetWhereFirst(l => l.TableName.ToLower() == LoadSubType.TLIloadOtherLibrary.ToString().ToLower());
                            TLIloadOtherLibrary loadOtherLibrary = _mapper.Map<TLIloadOtherLibrary>(LoadOtherLibraryViewModel);
                            var test = false;
                            string CheckDependencyValidation = CheckDependencyValidationForLoadOther(LoadOtherLibraryViewModel);

                            if (!string.IsNullOrEmpty(CheckDependencyValidation))
                            {
                                CheckDependencyValidation.ToLower();
                                char.ToUpper(CheckDependencyValidation[0]);
                                return new Response<AllItemAttributes>(true, null, null, CheckDependencyValidation, (int)ApiReturnCode.fail);
                            }

                            string CheckGeneralValidation = CheckGeneralValidationFunction(LoadOtherLibraryViewModel.TLIdynamicAttLibValue, TableNameEntity.TableName);
                            ...
                            if (test == true)
                            {
                                var CheckModel = _unitOfWork.LoadOtherLibraryRepository.GetWhereFirst(x => x.Model == loadOtherLibrary.Model && !x.Deleted);
                                if (CheckModel != null)
                                {
                                    return new Response<AllItemAttributes>(true, null, null, $"This Model {loadOtherLibrary.Model} is already exists", (int)ApiReturnCode.fail);
                                }
                                //else if (loadOtherLibrary.Length <= 0)
                                //{
                                //    return new Response<AllItemAttributes>(true, null, null, "Length Should be bigger than zero", (int)ApiReturnCode.fail);
                                //}
                                //else if (loadOtherLibrary.Width <= 0)
                                //{
                                //    return new Response<AllItemAttributes>(true, null, null, "Width Should be bigger than zero", (int)ApiReturnCode.fail);
                                //}
                                //else if (loadOtherLibrary.Height <= 0)
                                //{
                                //    return new Response<AllItemAttributes>(true, null, null, "Height Should be bigger than zero", (int)ApiReturnCode.fail);
                                //}
                                //else if (loadOtherLibrary.SpaceLibrary <= 0)
                                //{
                                //    return new Response<AllItemAttributes>(true, null, null, "SpaceLibrary Should be bigger than zero", (int)ApiReturnCode.fail);
                                //}
                                _unitOfWork.LoadOtherLibraryRepository.AddWithHistory(Helpers.LogFilterAttribute.UserId, loadOtherLibrary);
                                _unitOfWork.SaveChanges();

                                dynamic LogisticalItemIds = new ExpandoObject();
                                LogisticalItemIds = LoadOtherLibraryViewModel;

                                AddLogisticalItem(LogisticalItemIds, loadOtherLibrary, TableNameEntity.Id);

                                if (LoadOtherLibraryViewModel.TLIdynamicAttLibValue.Count > 0)
                                {
        ...
        public void AddLogisticalItem(dynamic LogisticalItemIds, dynamic CivilEntity, int TableNameEntityId)
        {
        ...
        public string CheckDependencyValidationForLoadOther(object Input)
        {
            AddLoadOtherLibraryViewModel AddLoadOtherLibraryViewModel = _mapper.Map<AddLoadOtherLibraryViewModel>(Input);

            List<DynamicAttViewModel> DynamicAttributes = _mapper.Map<List<DynamicAttViewModel>>(_unitOfWork.DynamicAttRepository
                .GetIncludeWhere(x => x.tablesNames.TableName.ToLower() == TablesNames.TLIloadOtherLibrary.ToString().ToLower() && !x.disable
            ...
            foreach (DynamicAttViewModel DynamicAttribute in DynamicAttributes)
            {
                ...
                if (Dependency != null)
                {
                    AddDynamicLibAttValueViewModel InsertedDynamicAttributeValue = AddLoadOtherLibraryViewModel.TLIdynamicAttLibValue
                    ...
                    foreach (int RowId in RowsIds)
                    {
                        ...
                        foreach (TLIrule Rule in Rules)
                        {
                            ...
                            if (Rule.attributeActivatedId != null)
                            {
                                ...
                                InsertedValue = AddLoadOtherLibraryViewModel.GetType().GetProperties()
                                    .FirstOrDefault(x => x.Name.ToLower() == AttributeName.ToLower()).GetValue(AddLoadOtherLibraryViewModel, null);
                            }
                            else if (Rule.dynamicAttId != null)
                            {
                                AddDynamicLibAttValueViewModel DynamicObject = AddLoadOtherLibraryViewModel.TLIdynamicAttLibValue
        ...
        #endregion
        public async Task<Response<AllItemAttributes>> DeletedLoadOtherLibrary(int Id)
        {
            using (TransactionScope transaction = new TransactionScope())
            {
                try
        ...
        public async Task<Response<AllItemAttributes>> DisableLoadOtherLibrary(int Id)
        {
            using (TransactionScope transaction = new TransactionScope())
            {
                try
                {
                    ...
                    var NewLoadOtherLibraryEntity = _unitOfWork.LoadOtherLibraryRepository.GetAllAsQueryable().AsNoTracking().FirstOrDefault(x => x.Id == Id);
        ...
        public async Task<Response<AllItemAttributes>> EditLoadOtherLibrary(EditLoadOtherLibraryViewModel LoadOtherLibraryViewModel)
        {
            using (TransactionScope transaction =
                new TransactionScope(TransactionScopeOption.Required,
                                   new System.TimeSpan(0, 15, 0)))
            {
                ...
                {
                    int resultId = 0;
                    var TableNameEntity = _unitOfWork.TablesNamesRepository.GetWhereFirst(l => l.TableName.ToLower() == LoadSubType.TLIloadOtherLibrary.ToString().ToLower());
                    TLIloadOtherLibrary LoadOtherLibrary = _mapper.Map<TLIloadOtherLibrary>(LoadOtherLibraryViewModel);
                    var CheckModel = _unitOfWork.LoadOtherLibraryRepository.GetWhereFirst(x => x.Model.ToLower() == LoadOtherLibrary.Model.ToLower() &&
                        x.Id != LoadOtherLibrary.Id && !x.Deleted);
                    var LoadOther = _unitOfWork.LoadOtherLibraryRepository.GetAllAsQueryable().AsNoTracking().FirstOrDefault(x => x.Id == LoadOtherLibraryViewModel.Id);

                    LoadOtherLibrary.Active = LoadOther.Active;
                    LoadOtherLibrary.Deleted = LoadOther.Deleted;
                    if (CheckModel != null)
                    {
                        return new Response<AllItemAttributes>(true, null, null, $"This Model {LoadOtherLibrary.Model} is already exists", (int)ApiReturnCode.fail);
                    }
                    ...
                    var testUpdate = _unitOfWork.TablesHistoryRepository.CheckUpdateObject(LoadOther, LoadOtherLibrary);
                    if (testUpdate.Details.Count != 0)
                    {
                        _unitOfWork.LoadOtherLibraryRepository.Update((TLIloadOtherLibrary)testUpdate.original);
                        // resultId = _unitOfWork.TablesHistoryRepository.AddHistoryForEdit(LoadOtherLibrary.Id, TableNameEntity.Id, "Update", testUpdate.Details.ToList());
                        await _unitOfWork.SaveChangesAsync();
                    }

                    string CheckDependency = CheckDependencyValidationForLoadOtherEditApiVersion(LoadOtherLibraryViewModel);
                    ...
                    string CheckValidation = CheckGeneralValidationFunctionEditApiVersion(LoadOtherLibraryViewModel.DynamicAtts, Helpers.Constants.TablesNames.TLIloadOtherLibrary.ToString());
                    ...
                    var CheckVendorId = _unitOfWork.LogisticalitemRepository
                        .GetIncludeWhereFirst(x => x.logistical.logisticalType.Name.ToLower() == Helpers.Constants.LogisticalType.Vendor.ToString().ToLower() &&
                            x.IsLib && x.tablesNamesId == TableNameEntity.Id && x.RecordId == LoadOtherLibrary.Id, x => x.logistical,
                    ...
                    EditLogisticalItem(LogisticalItemIds, LoadOtherLibrary, TableNameEntity.Id, OldLogisticalItemIds);

                    if (LoadOtherLibraryViewModel.DynamicAtts != null)
                    {
                        if (LoadOtherLibraryViewModel.DynamicAtts.Count > 0)

                        {
                            _unitOfWork.DynamicAttLibRepository.UpdateDynamicLibAttsWithHistory(LoadOtherLibraryViewModel.DynamicAtts, TableNameEntity.Id, LoadOtherLibrary.Id, Helpers.LogFilterAttribute.UserId, resultId, LoadOther.Id);
                        }
                    }
                    await _unitOfWork.SaveChangesAsync();
                    transaction.Complete();
        ...
        public string CheckDependencyValidationForLoadOtherEditApiVersion(object Input)
        {
            EditLoadOtherLibraryViewModel EditLoadOtherLibraryViewModel = _mapper.Map<EditLoadOtherLibraryViewModel>(Input);

            List<DynamicAttViewModel> DynamicAttributes = _mapper.Map<List<DynamicAttViewModel>>(_unitOfWork.DynamicAttRepository
                .GetIncludeWhere(x => x.tablesNames.TableName.ToLower() == Helpers.Constants.TablesNames.TLIloadOtherLibrary.ToString().ToLower() && !x.disable
                    , x => x.tablesNames).ToList());
            ...
            foreach (DynamicAttViewModel DynamicAttribute in DynamicAttributes)
            {
                ...
                if (Dependency != null)
                {
                    DynamicAttLibViewModel InsertedDynamicAttributeValue = EditLoadOtherLibraryViewModel.DynamicAtts
                    ...
                    foreach (int RowId in RowsIds)
                    {
                        ...
                        foreach (TLIrule Rule in Rules)
                        {
                            ...
                            if (Rule.attributeActivatedId != null)
                            {
                                ...
                                InsertedValue = EditLoadOtherLibraryViewModel.GetType().GetProperties()
                                    .FirstOrDefault(x => x.Name.ToLower() == AttributeName.ToLower()).GetValue(EditLoadOtherLibraryViewModel, null);
                            }
                            else if (Rule.dynamicAttId != null)
                            {
                                DynamicAttLibViewModel DynamicObject = EditLoadOtherLibraryViewModel.DynamicAtts
        ...
        public Response<AllItemAttributes> GetById(int Id)
        {
            try
            {
                ...
                TLItablesNames TableNameEntity = _unitOfWork.TablesNamesRepository.GetWhereFirst(c =>
                    c.TableName == Helpers.Constants.TablesNames.TLIloadOtherLibrary.ToString());

                List<BaseAttView> ListAttributesActivated = new List<BaseAttView>();

                TLIloadOtherLibrary LoadOtherLibrary = _unitOfWork.LoadOtherLibraryRepository.GetWhereFirst(x =>
                    x.Id == Id);

                ListAttributesActivated = _unitOfWork.AttributeActivatedRepository.GetAttributeActivated(TableNameEntity.TableName, LoadOtherLibrary, null).ToList();

                ListAttributesActivated.AddRange(_unitOfWork.LogistcalRepository.GetLogistical(Helpers.Constants.TablePartName.LoadOther.ToString(), TableNameEntity.TableName, Id));
        ...
        public Response<ReturnWithFilters<LoadOtherLibraryViewModel>> GetLoadOtherLibraries(ParameterPagination parameters, List<FilterObjectList> filters)
        {
            try
            {
                int count = 0;
                ReturnWithFilters<LoadOtherLibraryViewModel> Response = new ReturnWithFilters<LoadOtherLibraryViewModel>();
                var LoadOtherLibraries = _unitOfWork.LoadOtherLibraryRepository.GetAllIncludeMultiple(parameters, filters, out count, null).OrderBy(x => x.Id).ToList();
                var LoadOtherLibrariesModels = _mapper.Map<List<LoadOtherLibraryViewModel>>(LoadOtherLibraries);
                Response.Model = LoadOtherLibrariesModels;
                Response.filters = null;
                return new Response<ReturnWithFilters<LoadOtherLibraryViewModel>>(true, Response, null, null, (int)ApiReturnCode.success, count);
            }
            catch (Exception err)
            {

                return new Response<ReturnWithFilters<LoadOtherLibraryViewModel>>(true, null, null, err.Message, (int)ApiReturnCode.fail);
            }
        }
        ...
        public Response<ReturnWithFilters<object>> GetLoadOtherLibrariesWithEnableAtt(CombineFilters CombineFilters, ParameterPagination parameterPagination, bool? isRefresh)
        {
            try
            {
                if (_LoadOtherLibraryAllAttributesViewManagement == null || UnitOfWork.AllDynamicAttribute == null ||
                    (isRefresh != null ? isRefresh.Value : false))
                {
                    ...
                    _LoadOtherLibraryAttributeActivated = UnitOfWork.AllAttributeViewManagment.Where(x =>
                    ...
                    _LoadOtherLibraryDynamicAttributes = UnitOfWork.AllDynamicAttribute.Where(x =>
                        x.LibraryAtt && !x.disable &&
                        x.tablesNames.TableName == TablesNames.TLIloadOtherLibrary.ToString()).ToList();

                    _LoadOtherLibraryEntities = _unitOfWork.LoadOtherLibraryRepository
                        .GetWhere(x => !x.Deleted).ToList();

                    _LoadOtherLibraryAllAttributesViewManagement = UnitOfWork.AllAttributeViewManagment.Where(x =>
                ...
                {
                    OtherLoadLibraryAttribute = _LoadOtherLibraryAttributeActivated;
                }
                ...
                if (AttributeFilters != null && AttributeFilters.Count > 0)
                {
                    ...
                    //
                    List<TLIdynamicAtt> LibDynamicAttListIds = _LoadOtherLibraryDynamicAttributes.Where(x =>
                        ...
                        foreach (StringFilterObjectList LibraryProp in LibraryPropsAttributeFilters)
                        {
                            if (StringLibraryProps.Select(x => x.Name.ToLower()).Contains(LibraryProp.key.ToLower()))
                            {
                                Libraries = Libraries.Where(x => StringLibraryProps.AsEnumerable().FirstOrDefault(y => (LibraryProp.key.ToLower() == y.Name.ToLower()) && (LibraryProp.value.AsEnumerable().FirstOrDefault(w =>
                                     y.GetValue(_mapper.Map<LoadOtherLibraryViewModel>(x), null) != null ? y.GetValue(_mapper.Map<LoadOtherLibraryViewModel>(x), null).ToString().ToLower().StartsWith(w.ToLower()) : false) != null)) != null).AsEnumerable();
                            }
                            else if (NonStringLibraryProps.Select(x => x.Name.ToLower()).Contains(LibraryProp.key.ToLower()))
                            {
                                Libraries = Libraries.Where(x => NonStringLibraryProps.AsEnumerable().FirstOrDefault(y => (LibraryProp.key.ToLower() == y.Name.ToLower()) && (y.GetValue(_mapper.Map<LoadOtherLibraryViewModel>(x), null) != null ?
                                    LibraryProp.value.AsEnumerable().Contains(y.GetValue(_mapper.Map<LoadOtherLibraryViewModel>(x), null).ToString().ToLower()) : false)) != null).AsEnumerable();
                            }
                        }
                ...
                if (AfterConvertDateFilters != null ? AfterConvertDateFilters.Count() > 0 : false)
                {
                    ...
                    // Library Dynamic Attributes...
                    //
                    List<TLIdynamicAtt> DateTimeLibDynamicAttListIds = _LoadOtherLibraryDynamicAttributes.Where(x =>
                    ...
                    if (LibraryProps != null)
                    {
                        ...
                        {
                            Libraries = Libraries.Where(x => LibraryProps.Exists(y => (LibraryProp.key.ToLower() == y.Name.ToLower()) && ((y.GetValue(_mapper.Map<LoadOtherLibraryViewModel>(x), null) != null) ?
                                ((LibraryProp.DateFrom >= Convert.ToDateTime(y.GetValue(_mapper.Map<LoadOtherLibraryViewModel>(x), null))) &&
                                    (LibraryProp.DateTo <= Convert.ToDateTime(y.GetValue(_mapper.Map<LoadOtherLibraryViewModel>(x), null)))) : (false))));
                        }
                ...
                List<TLIattributeViewManagment> AllAttributes = _LoadOtherLibraryAllAttributesViewManagement;
                ...
                foreach (LoadOtherLibraryViewModel LoadOtherLibraryViewModel in LoadOthersLibraries)
                {
                    ...
                    // 
                    List<TLIdynamicAtt> NotDateTimeLibraryDynamicAttributes = _LoadOtherLibraryDynamicAttributes.Where(x =>
                    ...
                    // 
                    List<TLIdynamicAtt> LibraryDynamicAttributes = _LoadOtherLibraryDynamicAttributes.Where(x =>
                        x.DataType.Name.ToLower() == "datetime" &&
        ...
        //#region Add History
        //public void AddHistory(int Load_Other_Lib_Id, string historyType, string TableName)
        //{

        //    AddTablesHistoryViewModel history = new AddTablesHistoryViewModel();
        //    history.RecordId = Load_Other_Lib_Id;
