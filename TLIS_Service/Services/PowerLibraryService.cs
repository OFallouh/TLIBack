...
namespace TLIS_Service.Services
{
    public class PowerLibraryService : IPowerLibraryService
    {
        ...
        public PowerLibraryService(IUnitOfWork unitOfWork, IServiceCollection services,IMapper mapper, ApplicationDbContext DbContext)
        {
        ...
        public Response<ReturnWithFilters<object>> GetPowerLibrariesWithEnableAttributes(CombineFilters CombineFilters, ParameterPagination parameterPagination, bool? isRefresh)
        {
            try
            {
                    ...
                    (isRefresh != null ? isRefresh.Value : false))
                {
                    ...
                    _PowerLibraryEntities = _PowerLibraryEntities = _DbContext.TLIpowerLibrary
                        .AsNoTracking()
                ...
                if (AttributeFilters != null && AttributeFilters.Count > 0)
                {
                    ...
                    if (AttrLibExist)
                    {
                        ...
                        IEnumerable<TLIpowerLibrary> Libraries = _PowerLibraryEntities.AsEnumerable();
                    ...
                    WithoutDateFilterPowersLibraries = _mapper.Map<List<PowerLibraryViewModel>>(_PowerLibraryEntities.Where(x =>
                ...
                if (AfterConvertDateFilters != null ? AfterConvertDateFilters.Count() > 0 : false)
                {
                    ...
                    WithDateFilterPowersLibraries = _mapper.Map<List<PowerLibraryViewModel>>(_PowerLibraryEntities.Where(x =>
                        x.Id > 0 && IntersectLibraryIds.Contains(x.Id)).ToList());
                }
                ...
                {
                    PowersLibraries = _mapper.Map<List<PowerLibraryViewModel>>(_PowerLibraryEntities.Where(x =>
                        x.Id > 0).ToList());
                }
        ...
        public Response<AllItemAttributes> GetById(int Id, string TableName)
        {
            try
            {
                ...
                TLIpowerLibrary PowerLibrary = _unitOfWork.PowerLibraryRepository.GetWhereFirst(x =>
                    x.Id == Id);

                List<BaseAttView> ListAttributesActivated = _unitOfWork.AttributeActivatedRepository.GetAttributeActivated(TableName, PowerLibrary, null).ToList();

                ListAttributesActivated.AddRange(_unitOfWork.LogistcalRepository.GetLogistical(Helpers.Constants.TablePartName.Power.ToString(), TableName, Id));
        ...
        public async Task<Response<AllItemAttributes>> DisablePowerLibrary(string TableName, int Id)
        {
            try
            {
                ...
                _PowerLibraryEntities.FirstOrDefault(x => x.Id == Id).Active = NewPowerLibrary.Active;
