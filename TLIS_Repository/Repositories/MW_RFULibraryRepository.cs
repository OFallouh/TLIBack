...
{
    public class MW_RFULibraryRepository : RepositoryBase<TLImwRFULibrary, MW_RFULibraryViewModel, int>, IMW_RFULibraryRepository

    {
        private ApplicationDbContext _context = null;
        IMapper _mapper;

        public MW_RFULibraryRepository(ApplicationDbContext context, IMapper mapper) : base(context, mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public List<KeyValuePair<string, List<DropDownListFilters>>> GetRelatedTables()
        {
            ...
            var boardType = _context.TLIboardType.AsNoTracking().Where(x => !x.Disable && !x.Deleted).ToList();
