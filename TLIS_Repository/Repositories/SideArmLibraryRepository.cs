...
{
    public class SideArmLibraryRepository: RepositoryBase<TLIsideArmLibrary, SideArmLibraryViewModel, int>, ISideArmLibraryRepository
    {
        private readonly ApplicationDbContext _context;
        IMapper _mapper;
        public SideArmLibraryRepository(ApplicationDbContext context, IMapper mapper) : base(context, mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        public List<KeyValuePair<string, List<DropDownListFilters>>> GetRelatedTables()
        {
            ...
            var Suppliers = _context.TLIlogistical.AsNoTracking().Include(x => x.logisticalType).Include(x => x.tablePartName)
               .Where(x => x.Active && !x.Deleted && x.logisticalType.Name.ToLower() == Constants.TLIlogisticalType.Supplier.ToString().ToLower() && !x.logisticalType.Disable &&
                   !x.logisticalType.Deleted && x.tablePartName.PartName.ToLower() == Constants.TablePartName.SideArm.ToString().ToLower()).ToList();
